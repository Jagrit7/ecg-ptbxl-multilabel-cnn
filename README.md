...
## Architecture
Custom 1D-CNN with 7 convolutional blocks (tapered kernel sizes 15 → 3), each using 
BatchNorm + ReLU, with MaxPool for downsampling. Global Average Pooling replaces a 
flatten layer to keep the parameter count low, followed by a 2-layer FC head with 
dropout for the multi-label output.
...
- LR scheduler: `ReduceLROnPlateau` (monitors val macro-AUC)
...
3. Run `ecg_ptbxl_training.ipynb` top to bottom
